...
class Population:
	...
	def add_individual(self, child, generation=-1):
		#add individual with ID i
		self.generation_list[generation].append(child) #add indiv to most recent generation
	...
	def test_generation(self, test_func, fitness_arg, generation=-1):
		...
		for indiv in self.generation_list[generation]:
			score = test_func(indiv, fitness_arg)
			indiv.fitness = score
	...
	def choose_parents_percentile(self, top_percent, high_low='high', generation=-1):
		...
		'''
		ID_scores = [(i, indiv.fitness) for i, indiv in enumerate(self.generation_list[generation])]
		if high_low == 'high':
	...
	def create_next_generation(self, create_child_func, gen_size, mutation_rates):
		...
		for i in range(gen_size):
			...
			parent1_id = random.choice(self.parents_IDs_list)
			parent2_id = random.choice(self.parents_IDs_list)
			parent1 = self.generation_list[-2][parent1_id]
			parent2 = self.generation_list[-2][parent2_id]

			child = create_child_func(parent1, parent2, mutation_rates)
...
def test_indiv(individual, fitness_arg):

	nodes=get_nodes(individual)
	input_nodes=nodes[0]

	filled_tiles=fitness_arg[0]
	iterations=fitness_arg[1]
	...
	for i in range(iterations):
		...
		if board_state[action]==0:
			individual.fitness+=win_points
		elif board_state[action]!=0:
			individual.fitness+=lose_points

	return individual.fitness

#TODO: Make mutation rates list

def create_child(parent1, parent2, mutation_rates):
    
	addition_rate     = mutation_rates[0]
	subtraction_rate  = mutation_rates[1]
	substitution_rate = mutation_rates[2]
	crossover_rate    = mutation_rates[3]
...
mutation_rates = (0.1,0.1,0.1 ,0.9) #(addition_rate, subtraction_rate, substitution_rate, crossover_rate)
...
for gen in range(generations):
	...
	fitness_arg = (2, 10)
	population.test_generation(test_indiv, fitness_arg)
	#choose parents
